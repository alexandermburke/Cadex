...
export async function POST(request) {
  ...
  try {
    ...
    if (!citation || typeof citation !== 'string' || !citation.trim()) {
      ...
      return NextResponse.json(
        { error: 'Invalid or missing "citation" in request body.' },
        { status: 400 }
      )
    }
    ...
    if (detailed) {
      prompt = `
Generate an extremely comprehensive and detailed case summary for the following case title and citation. The summary should be as in-depth and accurate as Quimbee case briefs, and must include the following sections:
...
"${inputCitation}"
      ...
      prompt = `
Generate a comprehensive case summary based on the following case title and citation. The summary should include:
