...
export default function CaseApplication() {
    ...
    const placeholderTexts = {
        ...
        skillLevel: 'Select the skill level for the case',
        feedback: 'Enter feedback or notes about the case',
        attachments: 'No placeholder needed for attachments'
    };
        ...
        {sortDetails(Object.keys(caseMeta)).filter(val => val !== 'id' && val !== 'attachments' && val !== 'dateCreated').map((entry, entryIndex) => {
            return (
                <div className='flex items-center gap-4' key={entryIndex}>
                    <p className='capitalize font-medium w-24 sm:w-32'>{entry === 'caseTitle' ? 'Case Title' : entry === 'caseDescription' ? 'Case Description' : entry}</p>
                    {entry === 'skillLevel' ? (
    ...
    <input
        ...
        placeholder={placeholderTexts[entry] || 'Enter ' + entry}
        ...
        onChange={(e) => updateCaseData(entry, e.target.value)} />
                ...
                {caseMeta.id && (
                    <div className='grid grid-cols-3 gap-4 sm:w-fit'>
                        ...
                        <Link href={'/case/' + caseMeta.id} target='_blank' className={'flex items-center justify-center gap-2 border border-solid border-blue-950 bg-white p-4 rounded-full text-blue-950 duration-200 hover:opacity-50' + (!caseMeta.caseDescription || !caseMeta.caseTitle ? 'opacity-50' : '')}>
                            <p className=''>PDF Viewer</p>
                            <i className="fa-solid fa-arrow-up-right-from-square"></i>
                        </Link>
                        <button onClick={handleDeleteCase} className='flex items-center justify-center gap-2 border border-solid border-red-500 bg-white p-4 rounded-full text-red-500 duration-200 hover:opacity-75'>
