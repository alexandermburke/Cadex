    ...
    const handleRegistration = async () => {
        ...
        try {
            const userData = {
                username,
                email,
                password, // This should be hashed or handled securely in your backend
                billing: {
                    plan: 'Free', // Set the default plan to "free"
                },
            };

            // Simulate sending data to the backend or Firestore
            console.log('Registering user:', userData);

            // Call your backend API to save the user (replace this with an actual API call)
            // Example: await fetch('/api/register', { method: 'POST', body: JSON.stringify(userData) });
        ...
        <>
            <div className="flex flex-col gap-6">
                <h2 className={'text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-semibold text-center ' + poppins.className}>
                    Join <span className="goldGradient">CadexLaw.com</span>
                </h2>
                <p className="text-center">
                    Sign up for free, must have an email with a supported College or University.
                </p>
            </div>
            <div className="flex flex-col gap-4 text-base sm:text-lg">
                ...
                <div className="flex items-stretch gap-4 max-w-[600px] mx-auto w-full">
                    ...
                    <Button
                        ...
                        saving={loading ? 'Saving...' : submitting ? 'Submitting' : isVerifying ? 'Verifying username' : ''}
